...
impl CS2 {
    ...
    fn active_weapon_entity(&self, local: &Player) -> Option<usize> {
        let weapon_services: usize = self
            .process
    ...
    fn convar_float(&self, addr: Option<usize>, default: f32) -> f32 {
        addr.map(|a| self.process.read::<f32>(a + 0x58))
            .filter(|v| v.is_finite())
            .unwrap_or(default)
    }

    fn convar_bool(&self, addr: Option<usize>, default: bool) -> bool {
    ...
    fn compute_inaccuracy(&self, local_pawn: usize, weapon: usize, vdata: usize) -> f32 {
        let so = &self.offsets.seed_sync;
    ...
    fn scan_scene_node_for_model(&self, scene_node: usize) {
        ...
        let mut validated: Option<(usize, usize, i32)> = None;
        for off in (0x100..0x300).step_by(8) {
            let val: usize = self.process.read(scene_node + off);
            if val == 0 {
                continue;
            }
            ...
            let cmodel: usize = self.process.read(val);
            if cmodel == 0 {
                continue;
            }
            let render_meshes: usize = self
                .process
                .read::<usize>(self.process.read::<usize>(cmodel + 0x78));
            if render_meshes == 0 {
                continue;
            }
            let hitbox_data: usize = self.process.read(render_meshes + 0x150);
            if hitbox_data == 0 {
                continue;
            }
    ...
    fn real_hitbox_capsules(&self, target: &Player) -> Option<Vec<(Vec3, Vec3, f32, i32)>> {
        ...
        let bone_array: usize = self.process.read(
            scene_node
        ...
        let cmodel: usize = self.process.read(model_handle);
        if cmodel == 0 {
            hb_fail!(
                HB_WARN_CMODEL,
        ...
        let render_meshes: usize = self.process.read(self.process.read::<usize>(cmodel + 0x78));
        if render_meshes == 0 {
            hb_fail!(
        ...
        let array_ptr: usize = self.process.read(hitbox_data + 0x30);
        if array_ptr == 0 {
            hb_fail!(
                HB_WARN_ARRAY_PTR,
        ...
        for i in 0..count {
            ...
            let hb_base = array_ptr + (i as usize) * 0x70;
            ...
            let bone_base = bone_array + (bone as usize) * 32;
    ...
    pub fn seed_sync_prepare(&self, local: &Player) -> Option<SeedSyncState> {
        ...
        let vdata_raw: usize = self.process.read(weapon + vdata_off);
